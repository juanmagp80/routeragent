...
async function saveTaskActivity(req: any, task: any, selectedModel: any, cost: number, estimatedTime: number, taskType: string, responseText: string) {
    try {
        ...
        console.log('📝 [SAVE-ACTIVITY] Datos a guardar:', {
            user_id: userId,
            task_type: taskType,
            model_used: selectedModel.name,
            cost,
            tokens_used: activityData.tokens_used
        });

        // Intentar guardar en usage_logs
        const { data, error } = await supabase
            .from('usage_logs')
            .insert(activityData)
            .select();
        ...
        if (error) {
            console.error('❌ [SAVE-ACTIVITY] Error guardando en usage_logs:', error);
            
            // Si la tabla no existe, intentar crear o guardar en otra tabla
            if (error.message?.includes('relation') && error.message?.includes('does not exist')) {
                console.log('⚠️ [SAVE-ACTIVITY] Tabla usage_logs no existe, intentando con tasks...');
                
                // Intentar guardar en la tabla tasks como alternativa
                const taskData = {
                    user_id: userId,
                    task_type: taskType,
                    ...
                    status: 'completed',
                    tokens_used: activityData.tokens_used,
                    latency_ms: Math.floor(estimatedTime),
                ...
                if (taskError) {
                    console.error('❌ [SAVE-ACTIVITY] Error guardando en tasks:', taskError);
                } else {
                    console.log('✅ [SAVE-ACTIVITY] Actividad guardada en tasks:', taskData2?.[0]?.id);
                }
            }
        } else {
            console.log('✅ [SAVE-ACTIVITY] Actividad guardada en usage_logs:', data?.[0]?.id);
        }
