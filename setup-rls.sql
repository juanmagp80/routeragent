...
-- 5. Políticas para tabla USAGE_RECORDS
-- Los usuarios pueden ver sus propios records de uso
...
-- Los servicios pueden insertar records (usando service role)
DROP POLICY IF EXISTS "Service can insert usage records" ON usage_records;
CREATE POLICY "Service can insert usage records" ON usage_records
    FOR INSERT WITH CHECK (true); -- Service role puede insertar cualquier record

-- Verificar el estado de RLS
SELECT schemaname, tablename, rowsecurity 
FROM pg_tables 
WHERE schemaname = 'public' 
AND tablename IN ('users', 'api_keys', 'usage_logs', 'usage_records');
